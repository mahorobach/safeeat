...
if (allowedOriginSet.size > 0) {
  console.log(
    `CORS: ${allowedOriginSet.size} origin(s) → ${[...allowedOriginSet].join(', ')}`,
  );
} else {
  console.warn(
    'CORS: ALLOWED_ORIGINS が空です。GitHub Pages 等からは API をブロックします。',
  );
}
...
app.use((err, req, res, next) => {
  if (err.status === 413 || err.type === 'entity.too.large') {
    return res.status(413).json({
      ok: false,
      error:
        'リクエストが大きすぎます。画像を切り取るか、もう少し小さい写真で試してください。',
    });
  }
  next(err);
});
...
app.use((err, req, res, next) => {
  if (
    ...
    return res.status(400).json({
      ok: false,
      error:
        'リクエストの送信中にエラーが発生しました。電波状況を確認してもう一度試してください。',
    });
  }
  next(err);
});
