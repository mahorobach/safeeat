...
| AI解析（予備） | Claude API `claude-sonnet-4-5`系（テキスト判定・`/api/analyze`） |
...
| 3 | ユーザー認証（Supabase Auth）・月10回制限 | **完了** |
| 4 | UIデザイン改善・集客 | **← 現在** |
| 5 | サブスク課金（Stripe）・プラン管理 | 次（ユーザー数を見て判断） |
| 6 | グレー確認フロー | 有料プランの核心機能 |
| 7 | 判定履歴・お気に入り商品 | ユーザー体験向上 |
| 8 | iOSアプリ化（SwiftUI）※バーコード実装時 | App Store申請の前提 |
| 9 | App Store申請・リリース | 最終ゴール |
| 10 | Android対応 | 将来 |
| 11 | バーコードスキャン・商品DB | iOSアプリ化の主な動機 |
...
*作成日：2026年4月28日*
*最終更新：2026年5月3日（Phase 3認証完了・ロードマップ・プラン設計・収益方針を更新）*
