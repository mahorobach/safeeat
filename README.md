...
写真から成分を扱うときは、**切り取りはユーザーが手動**で行い、**文字起こし（読み取り）と成分判定を分けられる**ようにしています。

### ユーザー操作の流れ

1. **「📷 写真で読み取り」**で画像を選ぶ（任意で **✂ 原材料の範囲を選択**し、表示部分だけに切り取る）。
2. **「まず読み取りのみ」**にチェックを入れたうえで **「この画像で解析する」**を押すと、API は **成分テキストの抽出だけ**（`POST /api/analyze` の `type: "image"` かつ `extractOnly: true`）を実行する。  
   - 応答が短く、通信切れ・タイムアウトの切り分けに向く。  
   - 成功すると **テキストエリアに読み取り結果が入る**。
3. **📝 テキスト入力**タブで余計な行を削除・修正する（精度向上のため推奨）。
4. **「この内容で成分判定する」**（またはテキストタブで **「成分を解析する」**）で、**テキスト解析と同じ経路**（`type: "text"` の `ingredients`）によりオリエンタルベジ向けの分類を行う。

チェックを外した場合は従来どおり **画像 1 回で読み取り＋分類**（`extractOnly` なし）も可能です。

### 設計上の意図

| 観点 | 内容 |
|------|------|
| 切り取り | アプリが自動ではなく **ユーザーが原材料領域を指定**。撮り方のばらつきに対し、読み取り対象を明確にする。 |
| 読み取りプロンプト | パッケージ全体 OCR ではなく **原材料・成分表示フォーカス**（`server/routes/analyze.js` の extract 用プロンプト）。全体 OCR より目的に沿う。 |
| 判定 | 2 段階目は **常にテキスト API** と同じロジック＝手入力と品質を揃える。 |
