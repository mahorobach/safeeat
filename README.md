...
## 写真読み取りと解析の設計（固定 4 ステップ）

写真から成分を扱うときは **読み取りのみ → テキストで判定** が**必須**です（フロントから画像 1 回で読み取り＋分類を呼ぶ経路はありません）。タイムアウト・通信切断・巨大な JSON レスポンスを避けるための設計です。

### ユーザー操作の流れ

1. **「📷 写真で読み取り」**で画像を置く（ドラッグ＆ドロップまたはファイル選択）。
2. **（強く推奨）✂ 原材料の範囲を選択**で、成分表示だけを囲む。パッケージ全体・大きな余白のまま送ると失敗しやすいです。
3. **「この画像で読み取る」**（写真タブ表示時のメインボタン）で、API は **成分テキストの抽出だけ**（`POST /api/analyze` の `type: "image"` かつ `extractOnly: true`）を実行する。成功すると **テキストエリアに読み取り結果が入る**。
4. **📝 テキスト入力**で余計な行を削除・修正する。
5. **「この内容で成分判定する」**またはテキストタブの **「成分を解析する」**で、`type: "text"` の **`ingredients`** によりオリエンタルベジ向けの分類を行う。

サーバー側では引き続き `extractOnly` なしの画像解析（`analyzeFromImageSingleCall`）が定義されていますが、**このリポジトリの公式フロントでは使いません**（API クライアントや curl 用）。

### 設計上の意図

| 観点 | 内容 |
|------|------|
| 切り取り | **ユーザーが原材料領域を指定**。送信サイズと読み取り対象を絞り、エラー率を下げる。 |
| 読み取り | 常に **`extractOnly`**（短文・JSON は `ingredientListRaw` のみ）。 |
| 判定 | **常にテキスト API** と同じロジック＝手入力と品質を揃える。 |
| フロント設定 | `web/site-config.js` の `API_BASE` のみ（キーはサーバーの `CLAUDE_API_KEY`）。 |
